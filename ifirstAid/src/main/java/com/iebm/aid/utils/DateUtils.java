...
public class DateUtils {
	...
	/**
     * 判断日期 是今天0 昨天-1 前天-2 一月内-3 更早1
     * Add comments here.
     * @param date
     * @return
     */
    public static int judgmentDate(Date date) {
        String paramDate=DateUtils.dateToDateString(date, DATE_FORMAT);
        Date tempDate= getDate(paramDate, DATE_FORMAT);
        Calendar calendar=Calendar.getInstance();
        String d = calendar.get(Calendar.YEAR)+"-"+(calendar.get(Calendar.MONTH)+1)+"-"+calendar.get(Calendar.DATE);
	 ...
	 */
	public static String alterDateByDynamic(String dateStr, String inPattern,
			String outPattern) {
		if (StringUtils.isEmpty(dateStr)) {
			return "";
		}
...
//		sdf.setTimeZone(TimeZone.getDefault());
	...
	public static String getCurrDateNextMinute(String currDate, int minute) {
		try {
			DateFormat sdf = getDateFormat(TIMEF_FORMAT);
			java.util.Date result = sdf.parse(currDate);
			Calendar cal = Calendar.getInstance();
			cal.setTime(result);
	...
	public static java.util.Date getDateByFormat(String dateStr, String format) {
		try {
			if (StringUtils.isEmpty(dateStr)) {
				return null;
			}
...
//		Calendar now = Calendar.getInstance(TimeZone.getDefault());
	...
	public static java.sql.Date getSqlDate(String dateTimeStr) {
		...
		java.sql.Date d = new java.sql.Date(DateUtils.getDate(dateTimeStr,
				DateUtils.TIME_STR_FORMAT).getTime());
	...
	public static String getSelectMonth() {
		...
		for (int i = 0; i < 12; i++) {
			...
			optionHtml = optionHtml + "<option value=" + "" + nowdate + ""
					+ ">" + nowdate + "</option>";
		}
		return optionHtml;
	}
	 ...
	 */
	public static boolean dateBetween(String dateStr1, String dateStr2,
			String pattern) {
	...
	public static Date getAfterDate(String year, String month) {
		...
		String hours = "235959";
		if ("12".equals(month)) {
			time = String.valueOf((Integer.parseInt(year) + 1)) + "0101"
					+ hours;
		} else {
			String str1 = "";
	...
	public static int getMonthHasDays(int year, int month) {
		...
		// int result = Arrays.binarySearch(singleMonth, month);
		if (Arrays.binarySearch(singleMonth, month) >= 0) {
			...
			// 对于2月份的天数处理,判断是否为闰年
			if (DateUtils.YearIsLeapYear(year)) {
				monthDays = 29;
			} else {
				monthDays = 28;
			}
	...
	public static String getConverNumValue(double siceNum) {
		...
		if (ePlace != -1) {
			...
			ePrefix = ePrefix.substring(0, ePrefix.indexOf("."))
					+ ePrefix.substring(ePrefix.indexOf(".") + 1); // 过滤小数点
			...
			if (k > 0) {
				// 可以将小数点去掉，且需要补0
				String zeroStr = String.valueOf((long) Math.pow(10, k))
						.replaceAll("1", "");
		...
		if (resultStr.indexOf(".") != -1) {
			String str = resultStr.substring(resultStr.indexOf(".") + 1,
					resultStr.length());
	 ...
	 */
	public static java.util.Date getSpecialDate(Date date,
			String returnDateStyle, String transFormatStyle)
			throws ParseException {
		Date dateStr = null;
		DateFormat sdf = null;
	...
	public static String OverflowHidden(String str, int len) {
		if (StringUtils.isEmpty(str) || str.getBytes().length <= len) {
			return str;
		}
		byte b[] = new byte[len];
		for (int i = 0; i < len; i++) {
	 ...
	 * 
	 * @param i
	 *            ,正数:i=2表示当前月份的 后2个月 ; 负数:i=-1表示当前月份的 前1个月
	 * @param yearMonth
	 *            ,要比较的年月:如200809
	 ...
	 */
	public static boolean getAfterMonthAndDay(Date date, String month,
			String days) {
		int m = 0;
		int d = 0;
	 ...
	 * @return
	 */
	public static Map<String, Integer> getYYMMDDByDateStr(String dateStr,
			String separate) {
	...
	public static Map<String, String> getDayDate() {
		...
		sdate = cal.get(Calendar.YEAR) + "-" + month + "-"
				+ cal.get(Calendar.DAY_OF_MONTH);
		edate = cal.get(Calendar.YEAR) + "-" + month + "-"
				+ cal.get(Calendar.DAY_OF_MONTH);
		map.put("sdate", sdate);
		map.put("edate", edate);
		return map;
	...
	public static Map<String, String> getWeekDate() {
		...
		Map<String, String> map = new HashMap<String, String>();
		startcal.set(Calendar.DAY_OF_MONTH, cal.get(Calendar.DAY_OF_MONTH)
				- cal.get(Calendar.DAY_OF_WEEK) + 1); // 本周第一天时间
		endcal.set(Calendar.DAY_OF_MONTH,
				startcal.get(Calendar.DAY_OF_MONTH) + 6); // 本周最后一天日期
	 ...
	 * @return
	 */
	public static boolean dateBetweenOfMinute(Date startday, Date endday,
			int num) {
	 ...
	 *            传入日期格式
	 * @param conut
	 * @param returnFormatStr
	 *            返回格式 ，默认：yyyy-MM-dd
	 * @return yyyy-MM-dd 类同数据库格式
	 * @throws ParseException
	 */
	public static String getLastOrNextMonth(String dataTime, String formatStr,
			int conut, String... returnFormatStr) {
		try {
